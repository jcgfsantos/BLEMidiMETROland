...
/** Example with Display and showing volume

   Shows the metronome beat on the LILYGO® TTGO T5 V2.3 2.13 Inch E-Paper Screen
...
static uint16_t xb = 0;
static uint16_t yb = 0;
static uint8_t vol=0;
static uint16_t xv = 10;
static uint16_t yv = 122-7;
...
void sysexid(){
  ...
  if (addr.equals("0100010F")){
    ...
    u8g2Fonts.setFont(u8g2_font_9x15B_mn);            // select u8g2 font from here: https://github.com/olikraus/u8g2/wiki/fntlistall   //u8g2_font_7x14B_mf

    vol=midibuff[14];
    
    String svol="";
    svol=svol+(vol < 100 ? " " : "");  
    svol=svol+String(vol);
    
    u8g2Fonts.setCursor(xv, yv);                          // start writing at this position
    u8g2Fonts.print(svol);
    display.update(); 
  }
}
...
bool connectToServer() {
  ...
  if (!pClient->isConnected()) {
    ...
    while(!isconnected){
      ...
      if (pClient->isConnected()) {
        isconnected=true;
        u8g2Fonts.setFont(u8g2_font_open_iconic_all_2x_t);    // select u8g2 font from here: https://github.com/olikraus/u8g2/wiki/fntlistall
        u8g2Fonts.setCursor(10,30);
        u8g2Fonts.print("^");
...
void setup () {
  ...
  xb = display.width() / 2 - 40 ;
  yb = display.height() / 2+10;
  ...
  */
  ...
  getheadtmps(&header,&tmestamp);
  uint8_t midiMessage7[]={header,tmestamp,0xF0,0x41,0x10,0x00,0x00,0x00,0x28,0x12,0x01,0x00,0x03,0x09,0x00,0x3C,0x37,0xF7};
  ...
  getheadtmps(&header,&tmestamp);
  uint8_t midiMessage8[]={header,tmestamp,0xF0,0x41,0x10,0x00,0x00,0x00,0x28,0x12,0x01,0x00,0x02,0x13,0x28,0x42,0xF7};
